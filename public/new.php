...
if (isset($_POST["title"])) {
    ...
    $cover = ImageConstants::DEFAULT_COVER_FILENAME;
    $hasErrors = false;
    ...
    (new Book)
        ...
        ->create();
    $_SESSION["message"] = "Book created succesly.";
            ...
            <form action="<?= $_SERVER["PHP_SELF"] ?>" method="POST" enctype="multipart/form-data" novalidate>
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="title" id="title" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" placeholder="Enter title book" required>
                        <?= SessionErrorDisplay::showError("title") ?>
                    </div>
                    <div>
                        <div class="flex items-center space-x-6 pt-2">
                            <div class="shrink-0">
                                <img id="preview_cover" class="h-16 w-16 object-cover rounded-full" src="<?= "img/books/" . ImageConstants::DEFAULT_COVER_FILENAME ?>" alt="Cover preview">
                            </div>
                            <label class="block">
                    ...
                    <div>
                        ...
                        <select id="authorId" name="authorId" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" required>
                            ...
                            <?php foreach (Author::read() as $author): ?>
