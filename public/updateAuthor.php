...
if (!$id || !$author = Author::getAuthorById($id)) Navigation::redirectTo("authors.php");
if (isset($_POST["name"])) {
    ...
    if ($hasErrors) Navigation::redirectTo("updateAuthor.php?id=" . $author["id"]);
    if (
    ...
    ) Navigation::redirectTo("updateAuthor.php?id=" . $author["id"]);
    (new Author)
        ->setName($name)
        ->setSurname($surname)
        ...
        ->setCountry($country)
        ->update($id);
    if ($author["image"] !== $image) AuthorValidator::deleteOldImage($author["image"]);
    $_SESSION["message"] = "Author updated succesly.";
