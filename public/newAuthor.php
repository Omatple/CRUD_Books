...
if (isset($_POST["name"])) {
    ...
    $image = ImageConstants::DEFAULT_AUTHOR_IMAGE_FILENAME;
    $hasErrors = false;
    if (!AuthorValidator::isValidName($name)) $hasErrors = true;
    if (!AuthorValidator::isValidSurname($surname)) $hasErrors = true;
    if (!$hasErrors && !AuthorValidator::isUniqueFullName($name, $surname)) $hasErrors = true;
    if (!AuthorValidator::isValidCountry($country)) $hasErrors = true;
    if (AuthorValidator::isSentAnyFile($imageCodeError) && !AuthorValidator::isValidImageAuthor($imageData)) $hasErrors = true;
    if ($hasErrors) Navigation::refresh();
    if (
        AuthorValidator::isSentAnyFile($imageCodeError) &&
        !AuthorValidator::canMoveImage($imageTmpName, $image = AuthorValidator::getUniqueNameImage($imageName))
    ) Navigation::refresh();
    (new Author)
        ->setName($name)
        ->setSurname($surname)
        ->setImage("img/authors/" . basename($image))
        ->setCountry($country)
        ->create();
    $_SESSION["message"] = "Author created succesly.";
...
<body class="bg-gray-50 dark:bg-gray-900 antialiased">
    <section class="bg-white dark:bg-gray-900">
        <div class="px-4 mx-auto max-w-2xl lg:py-16">
            <h2 class="mb-4 text-2xl font-bold text-gray-900 dark:text-white">Add a New Author</h2>
            <form action="<?= $_SERVER["PHP_SELF"] ?>" method="POST" enctype="multipart/form-data" novalidate>
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    <div class="sm:col-span-2">
                        <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Name</label>
                        <input type="text" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" placeholder="Enter name author" required>
                        <?= SessionErrorDisplay::showError("name") ?>
                        <?= SessionErrorDisplay::showError("fullName") ?>
                    </div>
                    <div class="sm:col-span-2">
                        <label for="surname" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Surname</label>
                        <input type="text" name="surname" id="surname" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" placeholder="Enter surname author" required>
                        <?= SessionErrorDisplay::showError("surname") ?>
                    </div>
                    <div class="sm:col-span-2">
                        <label for="country" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Country</label>
                        <input list="countries" name="country" id="country" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" required>
                        <datalist id="countries">
                            <?php foreach (Countries::cases() as $country): ?>
                                <option value="<?= $country->toString() ?>"><?= $country->toString() ?></option>
                            <?php endforeach; ?>
                        </datalist>
                        <?= SessionErrorDisplay::showError("country") ?>
                    </div>
                    <div>
                        <div class="flex items-center space-x-6 pt-2">
                            <div class="shrink-0">
                                <img id="preview_cover" class="h-16 w-16 object-cover rounded-full" src="<?= "img/authors/" . ImageConstants::DEFAULT_AUTHOR_IMAGE_FILENAME ?>" alt="Author preview">
                            </div>
                            <label class="block">
                ...
                <div class="mt-12">
                    <button type="submit" class="inline-flex items-center px-5 py-2.5 text-sm font-medium text-center text-white bg-blue-700 rounded-lg focus:ring-4 focus:ring-blue-200 dark:focus:ring-blue-900 hover:bg-blue-800"><i class="fa-solid fa-user-plus mr-2"></i>Add Author</button>
